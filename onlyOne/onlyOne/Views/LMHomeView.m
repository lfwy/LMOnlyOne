...
- (void)setUpHomeViewWithHomeModal:(LMHomeModal *)homeModal animated:(BOOL)animate {
    self.volLabel.text = homeModal.strHpTitle;
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:homeModal.strThumbnailUrl] completed:nil];
    self.titleLabel.text = [homeModal.strAuthor componentsSeparatedByString:@"&"][0];
    self.authorLabel.text = [homeModal.strAuthor componentsSeparatedByString:@"&"][1];
    NSString *marketTime = [LMBaseFun homeENMarketTimeWithOriginalMarketTime:homeModal.strMarketTime];
    ...
    self.contentView.attributedText = [[NSAttributedString alloc] initWithString:homeModal.strContent attributes:attribute];
    [self.contentView sizeToFit];
    
    self.contentBackImageView.image = [[UIImage imageNamed:@"contBack"] stretchableImageWithLeftCapWidth:20 topCapHeight:20];
    
    [self.likeButton setTitle:[NSString stringWithFormat:@"  %@", homeModal.strPn] forState:UIControlStateNormal];
